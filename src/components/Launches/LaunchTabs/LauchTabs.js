...
class TabsWrappedLabel extends Component {
 ...
 {contextValue => {
  ...
  if (contextValue.data) {
   ...
   const pastLaunches  = launches.filter(launch =>  launch.upcoming === false )
    const upcomingLaunches  = launches.filter(launch =>  launch.upcoming === true )
  ...
  {value === 'upcoming' && <LaunchTab launches={upcomingLaunches}/>}
  </>
  } else {
    content = (
      <div
